...
import { Clock3, IdCard, ShieldAlert } from "lucide-react";
import type { AccountStatus } from "@/lib/account";

/**
 * Shown in place of a gated form (quote page) while the account still needs
 * its ID uploaded, is pending review, or was declined.
 */
export function VerificationNotice({ status }: { status: AccountStatus }) {
  const declined = status === "rejected";
  const needsId = status === "unverified" || status === "incomplete";
  const Icon = declined ? ShieldAlert : needsId ? IdCard : Clock3;
    ...
    <div className="mx-auto max-w-2xl rounded-3xl border border-[#e4e9f2] bg-white px-8 py-14 text-center shadow-[0_1px_2px_rgba(4,22,47,0.04),0_18px_40px_-16px_rgba(4,22,47,0.14)]">
      <span
        className={`mx-auto grid size-14 place-items-center rounded-full ${
          declined ? "bg-red-50 text-red-500" : needsId ? "bg-[#eef3fb] text-[#1A0A53]" : "bg-amber-50 text-amber-500"
        }`}
      >
      ...
      <h2 className="mt-5 text-xl font-bold text-[#1A0A53]">
        {declined ? "Verification declined" : needsId ? "Verify your identity" : "Verification pending"}
      </h2>
      <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-[#5b6b82]">
        {declined
          ? "We couldn't verify your account details. Update your ID information and resubmit — you'll be able to continue once approved."
          : needsId
            ? "Upload your ID documents from your profile and our team reviews them. This form unlocks once you're approved."
      ...
      <div className="mt-7 flex flex-wrap items-center justify-center gap-3">
        {(declined || needsId) && (
          ...
          >
            {needsId ? "Verify your identity" : "Update details"}
          </Link>
        )}
        <Link
          href="/profile"

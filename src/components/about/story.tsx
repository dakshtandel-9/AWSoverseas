    ...
    <Section spacing="lg" tone="soft">
      <div className="grid gap-14 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20">
        ...
        <ol className="relative flex flex-col">
          ...
          />
          <motion.div
            className="absolute left-[7px] top-2 w-px origin-top bg-gradient-to-b from-[#033e8d] to-[#0fade8]"
            initial={{ scaleY: 0 }}
          ...
          {STAGES.map((s, i) => (
            ...
            >
              <span
                className="relative z-10 mt-1.5 size-3.5 shrink-0 rounded-full border-2 border-white bg-[#033e8d] shadow-[0_0_0_4px_#eef3fb]"
                style={i === STAGES.length - 1 ? { background: "#0fade8" } : undefined}
                aria-hidden
              />
              <div className="min-w-0">
                <span className="font-mono text-[11px] font-bold uppercase tracking-[0.2em] text-[#0489c2]">
                  {s.marker}
                </span>
                <h3 className="mt-1.5 text-lg font-bold text-[#06234d]">{s.title}</h3>

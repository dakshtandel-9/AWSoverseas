      ...
      <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((item, i) => {
          ...
          return (
            <motion.div
              key={item.title}
              className="group relative flex flex-col overflow-hidden rounded-2xl border border-[#e4e9f2] bg-white p-7 transition-all duration-300 hover:-translate-y-1 hover:border-[#01214a]/30 hover:shadow-[0_24px_48px_-24px_rgba(4,22,47,0.25)]"
            ...
            >
              <div className="flex items-start justify-between">
                <span className="grid size-12 shrink-0 place-items-center rounded-xl bg-[#eef3fb] text-[#01214a] transition-colors duration-300 group-hover:bg-[#01214a] group-hover:text-white">
              ...
              <h3 className="mt-5 text-lg font-bold text-[#01214a]">{item.title}</h3>
              ...
              <ul className="mt-5 flex flex-1 flex-col gap-2">
                {item.features.slice(0, 4).map((f) => (
                  <li key={f} className="flex items-start gap-2 text-[13px] text-[#2a3a52]">
                    <span className="mt-1.5 size-1 shrink-0 rounded-full bg-[#d72846]" aria-hidden />
                    <span>{f}</span>
                  </li>
                ))}
              </ul>

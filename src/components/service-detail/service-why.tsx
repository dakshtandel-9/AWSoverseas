    ...
    <Section spacing="md" tone="soft">
      ...
      >
        {data.items.map((item) => (
          <motion.span
            key={item}
            className="inline-flex items-center gap-2 rounded-full bg-white px-4 py-2.5 text-sm font-semibold text-[#002144] shadow-[0_1px_2px_rgba(4,22,47,0.04)] ring-1 ring-[#e4e9f2]"
            variants={{
              hidden: { opacity: 0, scale: 0.92 },
              show: { opacity: 1, scale: 1, transition: { duration: 0.4, ease: [0.16, 1, 0.3, 1] } },
            }}
          >
            <ShieldCheck className="size-4 text-maroon-admin" />
            {item}
          </motion.span>
        ))}
      </motion.div>

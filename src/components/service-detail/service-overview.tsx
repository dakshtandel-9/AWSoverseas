    ...
    <Section spacing="md">
      <div className="grid gap-10 lg:grid-cols-[1.1fr_1fr] lg:gap-16">
        <Reveal direction="up">
          ...
          <h2 className="mt-3 text-3xl font-bold text-[#01214a] sm:text-4xl lg:text-[2.25rem]">
            {data.title}
          </h2>
        ...
        >
          {data.features.map((f) => (
            <motion.li
              key={f}
              className="flex items-center gap-3 rounded-xl border border-[#e4e9f2] bg-white px-4 py-3.5 text-sm font-semibold text-[#01214a]"
              variants={{
                hidden: { opacity: 0, x: 16 },
                show: { opacity: 1, x: 0, transition: { duration: 0.45, ease: [0.16, 1, 0.3, 1] } },
              }}
            >
              <span className="grid size-6 shrink-0 place-items-center rounded-full bg-[#fcf0f2] text-[#8e1b2e]">

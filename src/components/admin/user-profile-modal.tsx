  ...
  return createPortal(
    <AnimatePresence>
      <motion.div
        ...
        exit={{ opacity: 0 }}
      >
        <motion.div className="absolute inset-0 bg-[#04162f]/60 backdrop-blur-sm" onClick={onClose} aria-hidden />
        ...
        >
          <div className="flex items-start justify-between gap-4 border-b border-[#e4e9f2] px-7 py-5">
            <div className="min-w-0">
              <p id="user-profile-modal-title" className="truncate text-lg font-bold text-[#06234d]">
                {name}
              </p>
            ...
            <button
              ...
              onClick={onClose}
              aria-label="Close"
              className="grid size-8 shrink-0 place-items-center rounded-full text-[#5b6b82] transition-colors hover:bg-[#eef3fb] hover:text-[#06234d]"
            >
              <X className="size-4" />
            </button>
          </div>
          ...
          <div className="px-7 py-6">
            ...
            <div className="mt-5 grid gap-x-8 gap-y-3 text-sm text-[#06234d] sm:grid-cols-2">
              <p>
                <span className="font-semibold">Phone:</span>{" "}
                {profile.phone ? (
                  <a href={`tel:${profile.phone.replace(/\s+/g, "")}`} className="text-[#0489c2] hover:underline">
                    {profile.phone}
                  </a>
                ) : (
                  "—"
              ...
              <p>
                ...
                <a href={`mailto:${profile.email}`} className="text-[#0489c2] hover:underline">
                  {profile.email}
                </a>
              </p>
              <p>
  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-1.5 rounded-full border border-[#e4e9f2] px-3.5 py-1.5 text-xs font-semibold text-[#5b6b82] transition-colors hover:border-[#0fade8] hover:text-[#0489c2]"
      >

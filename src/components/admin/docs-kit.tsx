...
export function DocSection({
  ...
  return (
    <section id={id} className="scroll-mt-24 border-b border-[#e4e9f2] py-10 first:pt-0 last:border-b-0">
      ...
      <h2 className="mt-2 text-xl font-bold text-[#1A0A53] sm:text-2xl">{title}</h2>
    ...
    <ol className="flex flex-col gap-4">
      {steps.map((step, i) => (
        <li key={step.title} className="flex gap-4">
          ...
          <div>
            <p className="text-sm font-semibold text-[#1A0A53]">{step.title}</p>
      ...
      <table className="w-full border-collapse text-sm">
        <thead>
          <tr className="bg-[#f6f8fc] text-left">
            <th className="w-1/3 px-4 py-2.5 font-semibold text-[#1A0A53]">{columns[0]}</th>
            <th className="px-4 py-2.5 font-semibold text-[#1A0A53]">{columns[1]}</th>
          </tr>
        </thead>
        <tbody>
...
export function Kbd({ children }: { children: ReactNode }) {
  return (
    <code className="rounded-md border border-[#e4e9f2] bg-[#f6f8fc] px-1.5 py-0.5 font-mono text-[12px] text-[#1A0A53]">
      {children}
    </code>
  );
}

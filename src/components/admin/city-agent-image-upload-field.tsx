...
/**
 * Standalone upload form (not nested inside the city agent form) so its own
 * useActionState dispatch can never collide with the save action on the same
 * page.
 */
export function CityAgentImageUploadField({
  value,
  onUploaded,
}: {
  value: string;
  ...
  const [state, formAction, pending] = useActionState(uploadCityAgentImageAction, initialState);
    ...
    <div className="flex flex-col gap-3">
      {value ? (
        <div className="relative w-full max-w-xs overflow-hidden rounded-xl border border-[#e4e9f2]">
          <Image src={value} alt="" width={320} height={320} className="h-56 w-full object-cover" />
          <button
            type="button"
            onClick={() => onUploaded("")}
        ...
        <form ref={formRef} action={formAction}>
          <label className="flex w-full max-w-xs cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-[#e4e9f2] bg-[#f6f8fc] px-4 py-8 text-center transition-colors hover:border-[#9e4953]">
            ...
            <span className="text-sm font-medium text-[#5b6b82]">
              {pending ? "Uploading…" : "Click to upload a city photo"}
            </span>
            {!pending && (
              <span className="text-xs text-[#94a3b8]">Square-ish, 800×800px or larger, under 8MB</span>
            )}
            <input
              type="file"
              name="image"

...
const REVIEWS = [
  {
    ...
    rating: 5,
    quote:
      "aws overseas impex found and verified three manufacturers for us in under two weeks, then caught a spec mismatch during pre-shipment inspection that would have cost us the whole order. That's the kind of sourcing support we couldn't get from a regular forwarder.",
  },
  {
    name: "Priya Mehta",
    role: "Import Manager",
    ...
    rating: 5,
    quote:
      "We switched from our previous forwarder after constant shipment delays. aws overseas impex delivered our textile cargo 3 days ahead of schedule, and the live tracking kept us informed every step of the way.",
  },
  {
    name: "David Okonkwo",
    role: "CEO",
    ...
    rating: 5,
    quote:
      "Having one team handle supplier verification, quality inspection, and freight meant we stopped managing two separate vendors. Transparent pricing at every stage, and our repeat orders speak for themselves.",
  },
];
  ...
  return (
    <Section tone="soft" spacing="lg" className="relative overflow-hidden">
      ...
      <div
        ...
        style={{
          backgroundImage:
            "radial-gradient(ellipse 55% 45% at 50% 0%, rgba(158, 73, 83,0.07) 0%, transparent 65%)",
        }}
      />
      ...
      <div className="relative mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {REVIEWS.map((r, i) => (
          <motion.div
            ...
            className="group relative flex flex-col overflow-hidden rounded-3xl bg-white p-7 shadow-[0_1px_2px_rgba(4,22,47,0.04),0_8px_24px_-8px_rgba(4,22,47,0.10)] ring-1 ring-[#e4e9f2] transition-shadow duration-300 hover:shadow-[0_8px_32px_-4px_rgba(4,22,47,0.16)] hover:ring-[#9e4953]/30"
          >
            {/* Left accent stripe */}
            <span className="absolute inset-y-0 left-0 w-[3px] origin-top scale-y-0 rounded-l-3xl bg-gradient-to-b from-[#002144] to-[#9e4953] transition-transform duration-300 group-hover:scale-y-100" />

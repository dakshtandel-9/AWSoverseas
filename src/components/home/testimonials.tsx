...
const REVIEWS = [
  {
    ...
    rating: 5,
    quote:
      "AWSoversea handled our entire sea freight corridor from Shanghai to Rotterdam without a single delay. Customs clearance was seamless — their team knew exactly what documentation was needed.",
  },
  {
    name: "Priya Mehta",
    role: "Import Manager",
    ...
    rating: 5,
    quote:
      "We switched from our previous forwarder after constant shipment delays. AWSOversea delivered our textile cargo 3 days ahead of schedule and the live tracking kept us informed every step of the way.",
  },
  {
    name: "David Okonkwo",
    role: "CEO",
  ...
  return (
    <Section tone="soft" spacing="lg" className="relative overflow-hidden">
      ...
      <div
        ...
        style={{
          backgroundImage:
            "radial-gradient(ellipse 55% 45% at 50% 0%, rgba(10,165,240,0.07) 0%, transparent 65%)",
        }}
      />
      ...
      <div className="relative mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {REVIEWS.map((r, i) => (
          <motion.div
            ...
            className="group relative flex flex-col overflow-hidden rounded-3xl bg-white p-7 shadow-[0_1px_2px_rgba(4,22,47,0.04),0_8px_24px_-8px_rgba(4,22,47,0.10)] ring-1 ring-[#e4e9f2] transition-shadow duration-300 hover:shadow-[0_8px_32px_-4px_rgba(4,22,47,0.16)] hover:ring-[#0fade8]/30"
          >
            {/* Left accent stripe */}
            <span className="absolute inset-y-0 left-0 w-[3px] origin-top scale-y-0 rounded-l-3xl bg-gradient-to-b from-[#033e8d] to-[#0fade8] transition-transform duration-300 group-hover:scale-y-100" />
            ...
            <div className="mt-6 flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="text-sm font-bold text-[#06234d]">{r.name}</p>
      ...
      >
        {SIGNALS.map((s) => (
          <div key={s.label} className="flex flex-col items-center gap-1.5 bg-white px-6 py-6 text-center">
            <s.Icon className="size-5 text-[#0fade8]" />
            <p className="font-heading text-2xl font-extrabold text-[#033e8d]">{s.metric}</p>

    ...
    >
      ...
      <div className="absolute left-5 top-1/2 z-10 hidden -translate-y-1/2 flex-col items-center gap-3 lg:left-8 md:flex">
        <button
          ...
          className="grid size-9 place-items-center rounded-full border border-ink/15 bg-ink/[0.06] text-ink/70 backdrop-blur-md transition-colors duration-200 hover:bg-ink/[0.12] hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#9e4953]"
        >
        ...
        <div className="flex flex-col items-center gap-2">
          {Array.from({ length: slideCount }).map((_, i) => (
            <button
              ...
              className="group relative w-2 rounded-full transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#9e4953]"
              style={{ height: i === index ? 24 : 8, background: i === index ? "#9e4953" : "rgba(1,33,74,0.2)" }}
            />
          ))}
        </div>
        ...
        <div className="pointer-events-auto flex items-center gap-2">
          {Array.from({ length: slideCount }).map((_, i) => (
            <button
              ...
              aria-label={`Go to slide ${i + 1}`}
              aria-current={i === index}
              className="group relative h-2 rounded-full transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#9e4953]"
              style={{ width: i === index ? 24 : 8, background: i === index ? "#9e4953" : "rgba(1,33,74,0.2)" }}
            />
          ))}
        </div>
      </div>

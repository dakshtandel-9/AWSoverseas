      ...
      <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {data.items.map((item, i) => {
          ...
          return (
            <motion.div
              key={item.title}
              className="group rounded-2xl border border-[#e4e9f2] bg-white p-7 transition-all duration-300 hover:-translate-y-1 hover:border-[#d72846]/30 hover:shadow-[0_18px_40px_-20px_rgba(3,62,141,0.25)]"
              initial={{ opacity: 0, y: 18 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, margin: "-50px" }}
              transition={{ duration: 0.5, delay: (i % 3) * 0.08, ease }}
            >
              <span className="grid size-12 place-items-center rounded-xl bg-[#eef3fb] text-[#01214a] transition-colors duration-300 group-hover:bg-[#01214a] group-hover:text-white">
